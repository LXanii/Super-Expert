...
bool ExpertStartPopup::setup(ExpertMapLayer* parent) {
    m_parent = parent;
...
void ExpertStartPopup::startRun(CCObject*) {
    m_parent->confirmExpertRun();
};

ExpertStartPopup* ExpertStartPopup::create(ExpertMapLayer* parent) {
    auto ret = new ExpertStartPopup;
    if (ret && ret->initAnchored(POPUP_WIDTH, POPUP_HEIGHT, parent)) {
        ret->autorelease();
        return ret;
    }
    delete ret;
