...
class $modify(PlayLayer) {
  ...
  bool init(GJGameLevel * level, bool first, bool second) {
    ...
    if (super_expert) {
      ...
      if (lives >= 99) {
        m_fields->lives_text->setPosition({m_fields->lives_text->getPositionX() + 5, m_fields->lives_text->getPositionY()});
      }
      log::info("{}", lives);
...
struct SuperExpertInfoLayer : Modify<SuperExpertInfoLayer, LevelInfoLayer> {
...
struct ExpertPauseLayer : Modify<ExpertPauseLayer, PauseLayer> {
...
struct ExpertCallback : Modify<ExpertCallback, CreatorLayer> {
  struct Fields {
...
struct ExpertEndLevelLayer : Modify<ExpertEndLevelLayer, EndLevelLayer> {
  void showLayer(bool p0) { // find whatever gets called when u hit the end
    ...
    if (super_expert) {
      ...
      if (ids == pl->m_level->m_levelID) {
        current_level++;
      }
      if (current_level < 15) {
        downloading = true;
      }
      if (Mod::get()->getSettingValue<bool>("coin-lives")) {
        lives += coin_lives;
      }
      coin_lives = 0;
    }
...
struct ExpertBaseGameLayer : Modify<ExpertBaseGameLayer, GJBaseGameLayer> {
  // 1329 user coin pickup | 0x19d100 address make sure to submit l8r
