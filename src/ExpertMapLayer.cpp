...
bool ExpertMapLayer::init() { // beware, this code is dog shit holy fuck
  ...
  if (current_level_display > 15) {
    current_level_display = 15;
  }
  dl_count = 0;
  ...
  if (lives >= 100) {
    ...
    lives_text_x->setPosition({lives_text->getPositionX() - 18, lives_text->getPositionY()}); // og pos
  }
...
void ExpertMapLayer::addMap() {
  ...
  // thanks chatgpt
  std::vector<CCPoint> stageCoordinates = {{-247, -16}, {-212, -15}, {-183, -28}, {-150, -20}, {-116, -32}, {-82, -25}, {-50, -17}, {-26, 3}, {7, 8}, {14, -18}, {36, -36}, {69, -27}, {106, -30}, {142, -28}, {174, -16}};

  std::vector<CCPoint> blankCoordinates = {{-231, -14}, {-198, -21}, {-167, -22}, {-135, -25}, {-99, -27}, {-66, -20}, {-37, -7}, {-10, 7}, {9, -3}, {23, -28}, {51, -30}, {87, -27}, {124, -28}, {159, -22}};
...
void ExpertMapLayer::downloadLevels() {
  ...
  islandSprite->setColor({0, 0, 0});
  map_deco->setVisible(false);
  if (skips != Mod::get()->getSettingValue<int64_t>("skips")) {
    skips_left->setString(fmt::format("Skips: {}/{}", skips, Mod::get()->getSettingValue<int64_t>("skips")).c_str());
  }
...
void ExpertMapLayer::onGoBack(CCObject*) {
  ...
  if (super_expert && !downloading) {
    createQuickPopup("End Expert Run", "Would you like to <cr>end</c> your <cp>expert run</c>?", "NO", "YES", [this](FLAlertLayer*, bool btn2) {
      if (btn2) {
        expertReset();
        this->onGoBack(nullptr);
